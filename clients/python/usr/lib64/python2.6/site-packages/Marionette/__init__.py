...
err_code = mc.execute('command.name', args....)
    ...
    def __init__(self, host=None, port=None, timeout=None, debug=None):
        ...
        * timeout (socket timeout, default 900)
        ...
        # set timeout
        if timeout != None: self.sock.settimeout(timeout)
    ...
    def execute(self, command, *args):
        ...
        if res == 0:
            ...
            raise MarionetteError(res)


class MarionetteError(Exception):
    def __init__(self, errcode):
        if errcode == 100:
            self.value = "100: Marionette config error. Check argument count, runas_user and dangerous characters."
        elif errcode == 101:
            self.value = "101: Marionette whitelist error. Command name was not foud in config."
        elif errcode == 102:
            self.value = "102: Command path error. Remote server could not find the specified command."
        elif errcode == 103:
            self.value = "103: Exit Status error. The command ran and exited with a non-zero status."
        else:
            self.value = "Unkown failure. Report this failure and open a bug."
